...
function bpCurl($url, $apiKey, $post = false) {
	...
	curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0); // was: 1); // verify certificate
	curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 0); // was: 2); // check existence of CN and verify that it matches hostname
...
function bpCreateInvoice($orderId, $price, $posData, $options = array()) {	
	...
	$response = bpCurl('https://test.bp:8088/api/invoice/', $options['apiKey'], $post);
...
function bpGetInvoice($invoiceId, $apiKey=false) {
	...
	$response = bpCurl('https://test.bp:8088/api/invoice/'.$invoiceId, $apiKey);
